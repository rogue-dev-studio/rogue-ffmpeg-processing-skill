...
FFmpeg media processing. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `ffmpeg-processing` or user asks by name
...
2. **Confirm tooling** - Verify `ffmpeg-processing` runtime/CLI/MCP is available; document install if missing.
